...
impl LanguageServer for Backend {
    ...
    async fn did_open(&self, params: DidOpenTextDocumentParams) {
        ...
        self.open_files.lock().unwrap().insert(document.uri.clone(), open_file);
    }

    async fn did_close(&self, params: DidCloseTextDocumentParams) {
        self.open_files.lock().unwrap().remove(&params.text_document.uri);
    }
                ...
                if let Some(range) = change.range {
                    ...
                    if range.start.line == range.end.line {
                        ...
                        if addition_lines.len() == 1 {
                            ...
                            let end_of_last_line = file.contents.lines[range.end.line as usize][(range.end.character as usize)..].encode_utf16();
                    ...
                    messages.push(format!("New contents: {:?}!", file.contents))
                }
            }
